		...
		var options = {
			...
			getresult: function(data) {
				...
				option = {
					...
					title: {
						text: 'Recent Cutting Speed： ' + speedList[0] + 'CM/S',
		...
		var options = {
			...
			getresult: function(data) {
				...
				option = {
					...
					legend: {
						//icon: 'vertical',
						data: ['Effective Availability', 'Cutting Time(CT)'],
						//align: 'center',
						// right: '35%',
						top: '0',
						textStyle: {
					...
					series: [{
						...
						data: [{
							...
							label: {
								normal: {
									formatter: function (params) {
										console.log(fomatFloat(Math.round(v2 / v3 * 100),2));
										return 'RT% last 8 hrs:' + fomatFloat(Math.round(v2 / v3 * 100),2) + '%'
									},
									textStyle: {
										color: '#aaa',
										fontSize: 12
							...
							label: {
								normal: {
									formatter: function(params) {
										...
										return 'RT/CT:' + fomatFloat(Math.round(v1 / v3 * 100),2) + '%'
									},
									textStyle: {
										color: '#aaa',
										fontSize: 12
					...
					series: [{
						...
						data: [{
							...
							label: {
								normal: {
									formatter: function(params) {
										return v3 + 'm/' + fomatFloat(Math.round(v2 / v3 * 100),0) + '%'
									},
									textStyle: {
										color: '#aaa',
										fontSize: 12
	...
	function RightRank(){
		...
		var options = {
			...
			getresult: function(data) {
				...
				var str = "<tr> <th scope=\"col\">Ranking</th> <th scope=\"col\">Pattern</th> <th scope=\"col\">times of use</th> <th scope=\"col\">Utilisation</th> </tr>";
				for (let i = 0; i < data.length; i++) {
					if (i < 5){
						data[i].marker = cutstr(data[i].marker, 16);
						str += "<tr> <td><span>" + (i+1) + "</span></td> <td>" + data[i].marker + "</td> <td>" + data[i].counter + "<br></td> <td>"
						+ fomatFloat(data[i].effection,0) + "%<br></td> </tr>";
					}
				}
				if (data.length == 0){
