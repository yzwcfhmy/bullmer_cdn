...
$( Echart = function() {
			...
			getresult: function(data) {
				...
				option = {
					...
					legend: {
						...
						orient: 'vertical',
						data: ['Leerfahrt', 'Nebenzeit', 'Bitefeed', 'Schneiden']
					},
					grid: {
						left: '3%',
						right: '4%',
					...
					xAxis: {
						type: 'category',
						data: ['MoveTime', 'SecondTime', 'Bitefeed', 'Cutting'],
						axisLabel: {
							color: 'rgba(255,255,255,.6)',
							interval: 0, // 强制显示所有标签
		...
		var options = {
			...
			getresult: function(data) {
				...
				option = {
					...
					title: {
						text: 'Letztes Schneidtempo： ' + speedList[0] + 'CM/S',
		...
		var options = {
			...
			getresult: function(data) {
				...
				option = {
					...
					xAxis: {
						type: 'category',
						data: ['Abort', 'Pause','Schneiden', 'Bitefeed', 'Nebenzeit'],
						axisLabel: {
							color: 'rgba(255,255,255,.6)',
							interval: 0, // 强制显示所有标签
							rotate: 30, // 标签旋转30度，防止重叠
		...
		var options = {
			...
			getresult: function(data) {
				...
				option = {
					...
					legend: {
						//icon: 'vertical',
						data: ['Effective Availability', 'Schneidzeit(SZ)'], // Laufzeit(min.)
						//align: 'center',
						// right: '35%',
						top: '0',
						textStyle: {
		...
		var options = {
			...
			getresult: function(data) {
				...
				option = {
					...
					"xAxis": [{
						"type": "category",
						data: ['0Uhr', '2Uhr', '4Uhr', '6Uhr', '8Uhr', '10Uhr', '12Uhr', '14Uhr', '16Uhr', '18Uhr', '20Uhr', '22Uhr'],
						axisLine: {
							lineStyle: {
								color: "rgba(255,255,255,.1)"
							}
							...
							label: {
								normal: {
									formatter: function(params) {
										...
										return 'Anteil letzter 8 Std.' + fomatFloat(Math.round(v2 / v3 * 100),2) + '%'
									},
									textStyle: {
										color: '#aaa',
										fontSize: 12
							...
							label: {
								normal: {
									formatter: function(params) {
										...
										return 'LZ/SZ:' + fomatFloat(Math.round(v1 / v3 * 100),2) + '%'
									},
									textStyle: {
										color: '#aaa',
										fontSize: 12
	...
	function DangQianBantu(){
		...
		var options = {
			...
			error:function () {
				console.log("error");
				document.getElementById("LayerNow").innerText = "The service offline";
			}
		};
		// 执行Ajax
		// setTimeout('sendRequest(options)',500);
	...
	function RightRank(){
		...
		var options = {
			...
			getresult: function(data) {
				...
				var str = "<tr> <th scope=\"col\">Ranking</th> <th scope=\"col\">Muster</th> <th scope=\"col\">Nutzung</th> <th scope=\"col\">Nutzrate</th> </tr>";
				for (let i = 0; i < data.length; i++) {
					if (i < 5){
